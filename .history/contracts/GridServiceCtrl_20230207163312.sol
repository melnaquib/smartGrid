...
contract GridServiceCtrl is ERC721Enumerable, Ownable {
    ...
    void disonnect(address subscriber, bool connected) onlyOwner() subscriptionNotPaid(subscriber) {
        subscribers[subscriber] = false;
    }

    connect (account subscriber) is_subscribed() onlyOwner {
        connected
        emit Connect(subscriber);
    }
